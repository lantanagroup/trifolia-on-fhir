...
export class NonFhirResourceService {
    ...
    public search(page: number = 1, sort: string = 'name', projection?: {[key: string]: any}, implementationGuideId?: string, resourceType?: string, name?: string, title?: string): Observable<Paginated<INonFhirResource>> {
        let url = '/api/nonFhirResources?page=' + encodeURIComponent(page) + '&' + '_sort=' + encodeURIComponent(sort) + '&';

        if(projection){
           url += `_projection=${encodeURIComponent(JSON.stringify(projection))}&`;
        }
    ...
    public  getByName(resource: INonFhirResource, implementationGuideId?: string): Observable<INonFhirResource> {
      let url = `/api/nonFhirResources/name/${resource.type}`;
    ...
    public  getByType(resource: INonFhirResource, implementationGuideId?: string): Observable<INonFhirResource> {
      let url = `/api/nonFhirResources/type/${resource.type}`;
