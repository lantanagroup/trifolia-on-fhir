  ...
  async up(db, client) {
    ...
    const migrate = async function(extensionUrl, type) {
      ...
      for (const igRes of igResults) {
        ...
        const IgnoreWarningsReference = existingExtension.valueReference ? existingExtension.valueReference.reference : '';
        if (!IgnoreWarningsReference.startsWith('#')) continue;
        ...
        if(existingContained) {
          ...
          let coding = typeContained.coding.find(c => c.code === "ignore-warnings");
          if(!coding){
            continue;
          }
          ...
          // remove this ignore warnings from the current IG resource's extension list and contained list and add the new reference
    ...
    // migrate IG Ignore Warnings
    await migrate('https://trifolia-fhir.lantanagroup.com/StructureDefinition/extension-ig-ignore-warnings', 'IgnoreWarnings');
  },
